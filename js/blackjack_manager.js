...
function new_game(){
    ...
    // Enabled só no final da animação
    setTimeout(() => { 
    ...
    // "Deal Again", não reinicia o dinheiro do Jogador
...
function shuffleAnimation(){
    for (let i = 0 ; i < game.deck.length ; i++){
        ...
        }, 200); // Espera este tempo antes de começar
        setTimeout(() => { 
...
function play() { 
    ...
    if (!audio) {
        myaudio.play(); 
        audio = true;
        audioButton.src = './imagens/music_note.svg';
    } else {
        myaudio.pause();
        audio = false
        audioButton.src = './imagens/music_note_off.svg';
    }
};